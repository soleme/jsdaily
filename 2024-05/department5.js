import {defineProps} from "../js/vue.esm-browser";
...
let cust_orgLevel = [];
let childOrgs = [];
for(let i=0; i < deptData.d.results.length; i++){
    _departments.push(deptData.d.results[i].externalCode);
    childOrgs.push(deptData.d.results[i].externalCode);

    if(deptData.d.results.length > 0){
        if(deptData.d.results[0].cust_orgLevel === '2'){
